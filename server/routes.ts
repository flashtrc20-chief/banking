...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/wallets/:userId", requireNotRejected, async (req, res) => {
    try {
      ...
      // Return predefined wallets with $22.5M split for all users
      const getDefaultWallets = (uid: string) => {
        return [
          {
            id: `wallet_btc_${uid}`,
            userId: uid,
      ...
      // Return default wallets for all authenticated users
      res.json(getDefaultWallets(userId));
    } catch (error) {
  ...
  app.post("/api/transactions", requireNotRejected, async (req, res) => {
    try {
      ...
      // Validate minimum amount ($500,000)
      const amountNum = parseFloat(amount);
      if (amountNum < 500000) {
        return res.status(400).json({ 
          message: "Minimum flash amount is $500,000 USD" 
        });
      }
